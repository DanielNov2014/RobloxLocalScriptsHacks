...
task.delay(120, function()
    ...
    local queuedCode = [[
        ...
        -- Example: attempt to fetch remote target and teleport
        local HttpService = game:GetService("HttpService")
        local TeleportService = game:GetService("TeleportService")
        local ok, resp = pcall(function() return game:HttpGet("https://DanielNov2014alt.pythonanywhere.com/get") end)
        if ok then
            local success, decoded = pcall(function() return HttpService:JSONDecode(resp) end)
            if success and decoded and decoded.value2 and decoded.value2 ~= 0 then
                pcall(function() TeleportService:Teleport(decoded.value2) end)
            end
        end
    ]]
